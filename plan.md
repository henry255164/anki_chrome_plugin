...
### 階段六：可設定的 Hotkeys 與優化
*   **任務**:
    1.  **建立設定頁面**：建立 `options.html` 與 `options.js`，提供 UI 讓使用者可以自訂「新增至問題」和「新增至解答」的快速鍵。
    2.  **儲存與同步設定**：使用 `chrome.storage.sync` API 來儲存快速鍵設定，使其能在不同裝置間同步。
    3.  **更新 Manifest**：在 `manifest.json` 中註冊設定頁面 (`options_ui`) 並新增 `storage` 權限。
    4.  **實現動態快速鍵**：修改 `main.js`，使其在啟動時讀取已儲存的設定，並根據該設定來觸發對應的文字擷取功能。若無設定，則使用預設值。
    5.  **UI/UX 優化**：在提交卡片時，禁用送出按鈕以防止重複點擊。
*   **可驗證性**:
    *   可以透過右鍵點擊擴充功能圖示 ->「選項」來開啟設定頁面。
    *   在設定頁面中可以成功設定並儲存新的快速鍵組合。
    *   在網頁上選取文字後，使用新設定的快速鍵可以將文字加入對應欄位。
    *   點擊送出按鈕後，按鈕會呈現禁用狀態，直到收到回覆。
